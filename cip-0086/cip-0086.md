...
| **Acceleration Bonus** | +0 Bonus if delivered +180 Days from SV CIP Approval; +2 Bonus if delivered +60 Days from SV CIP Approval; Linear sliding scale between +60 and +180 days (Example: If delivered in +120 days, Chainsafe is awarded +1 SV Weight). *At least one asset must be live, having completed a transaction on MainNet* | +180 Days from CIP Approval | Max +2 |
| **Adoption Bonus** | *Driving TVL of assets on Canton.* +0.5 per asset with > $10m of TVL issued on MainNet (Max +2) | +180 Days from Middleware going to MainNet | Max +2 |
