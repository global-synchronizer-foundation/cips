...
except for [one limitation](#canton-coin-implementation) that is expected to be removed in the future.

In the remainder of this specification, we first provide an overview of the APIs and
the functionality they enable. We then provide technical details. The complete
implementation is referenced in the [Implementation](#implementation) section.
...
The Daml and OpenAPI definitions for all six APIs are part of the implementation
referenced below in the [Implementation](#implementation) section. Daml
...
The code referenced in the [Implementation](#implementation) section provides
...
Five of the six APIs concern asset registries; the exception is the
"allocation request API", which is implemented by apps rather than registries.
Asset registries can in principle freely choose which of these five APIs they
want to implement. However, they should implement all five of them to maximize
the utility of their tokens. An exception to this recommendation is asset
...
to provide maximum freedom for wallets and apps to fetch these.
...
by [querying the metadata](#storing-generic-metadata-in-cns-10-entries) of the CNS entry for the registry's
`admin` party with the key `splice.lfdecentralizedtrust.org/registry-urls`, and
...
* **2026-09-09:** Fixed stale references to the removed implementation proposal section to point to
  the Implementation section, and updated the limitation count in the Specification introduction.
