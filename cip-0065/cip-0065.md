...
* Add Layer Zero, Wormhole, and Chainlink as SVs at Weight 4 each 
* Create a bonus pool of Weight 2.5
...
| Wormhole integration with Canton Network | • Assets issued on the Canton Network can be bridged to any network supported by Wormhole • Assets issued on any network supported by Wormhole can be bridged to Canton <br> • Wormhole supports the Canton Network Token Standard | + 12 Months | 2 |
| Chainlink’s Cross Chain  Interoperability Protocol (CCIP) integration to Canton | • Chainlink enables CCIP service to connect the Canton Network Global Synchronizer with over 50 public and permissioned blockchains, at the highest levels of security and  reliability. | + 12 Months | 2 |
| Adoption (all applicants) | • Through GTM, education,  influence, and its efforts routes an increasing amount of assets and applications to Canton | + 180 Days  from Product Integration | 0.5 per 5  assets/applica tions using  Canton via  the bridge <br><br>Up to a Total of 2 ea |
| First to Production Incentive | • Once product is live, the first bridge to move $10m of assets to/from Canton Network will receive an SV Weight bonus starting at the time of the the incentive is met | + 180 Days  from Product Integration | 2.5 |
| First to Scale Incentive | • Once product is live, the first bridge to move $50m of assets to/from Canton Network will receive the default asset CNS entries | + 180 Days  from Product Integration | Default CNS Entries |
